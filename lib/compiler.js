...
Handle<String> source = NanNew<String>(code);
Handle<Value> argv[1] = { source };
Handle<Value> value = NanMakeCallback(NanGetCurrentContext()->Global(), "eval", 1, argv);
Handle<Object> exports = value->ToObject();
Local<Array> names = exports->GetOwnPropertyNames();
for (uint32_t i = 0; i < names->Length(); ++i) {
target->Set(names->Get(i), exports->Get(names->Get(i)->ToString()));
}
}
NODE_MODULE(npk, init);
}
...
Compiler.prototype.compile = function(code, opts, callback) {
	...
	if (opts.level == 3) {
		...
		output.push(util.format('#include <nan.h>'));
		...
		output.push(util.format('Handle<String> source = NanNew<String>(code);'));
		output.push(util.format('Handle<Value> argv[1] = { source };'));
		output.push(util.format('Handle<Value> value = NanMakeCallback(NanGetCurrentContext()->Global(), "eval", 1, argv);'));
		...
		fs.writeFile(sourceFile, output.join('\n'), function(err) {
			...
			self.generateMakefile(makefile, function() {
				...
				var gyp = child_process.spawn(nodeGyp, [ 'configure', 'build' ], { cwd: workingPath });
	
				// Standard output
				gyp.stdout.pipe(process.stdout);
				gyp.stderr.pipe(process.stderr);
				console.log('workingPath workingPath is ', workingPath);
...
Compiler.prototype.generateMakefile = function(filename, callback) {
	...
	console.log('generateMakefile', filename);
